...
//     // Cálculo fixo: a campanha tem 1,5 BNB e 500 números, logo cada número custa:
//     const fixedPricePerNumber = 1.5 / 500; // 0.003 BNB por número
  
//     // Variável para a cotação atual do BNB (em USD), obtida via CoinGecko
...
//     // Função para buscar a cotação atual do BNB e atualizar os valores na página
...
//              currentBNBValue = 300;
//            }
//            // Atualiza o preço por número (em BNB) e seu equivalente em USD
...
//     // Atualiza a contagem e o resumo da seleção (alinhado à direita)
...
document.addEventListener("DOMContentLoaded", function() {
  ...
  // Variável para armazenar o preço atual do BNB (em USD)
  let currentBNBValue = 0;
  ...
  // Função para buscar a cotação atual do BNB a partir da API da Binance
  function updateBNBPrice() {
  ...
  // Atualiza a contagem e o resumo da seleção
  function updateSelectedCount() {
    const count = selectedNumbers.length;
    selectedCountElem.innerText = count;
    const totalBNBForSelection = (count * fixedPricePerNumber).toFixed(6);
